...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (!user) {
            await interaction.editReply('Veuillez d\'abord connecter la cible avec la commande /forcelogin.');
            return;
        }
        if (await groups_users.findOne({ where: { user: target.id } })) {
            await interaction.editReply('La cible êst déjà associé à un groupe.');
            return;
        }
        ...
        selectCollector.on('collect', async i => {
            ...
            if (i.customId === 'selectYear') {
                ...
                await handleYearSelection(interaction, licenceName, licenceYear, target);
            }
        });
    },
};
async function handleYearSelection(interaction, licenceName, licenceYear, target) {
    // Votre logique ici
    const user = await users.findOne({ where: { discordid: target.id } });
    ...
    let newCategory = await createCategory(interaction, licenceYear + " - " + licenceName, target);
    await licences.update(
        { id: newCategory.id },
        { where: { name: licenceName, year: licenceYear.slice(-1) } }
    );
    await createChannels(interaction, list_group, newCategory, target);

    await interaction.editReply(`Groupe assigné`);
}
async function createCategory(interaction, categoryName, target) {
    ...
    category.permissionOverwrites.create(target, { ViewChannel: true });

    return category;
}

async function createChannels(interaction, listOfChannel, categoryParent, target) {
    for (let [course, group] of Object.entries(listOfChannel)) {
        ...
        await groups_users.create({ user: target.id, group: channel.id });
